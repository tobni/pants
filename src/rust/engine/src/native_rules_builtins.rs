...
pub async fn find_ancestor_files(request: AncestorFilesRequest) -> NodeResult<AncestorFiles> {
    ...
    let final_digest = if request.input_files.is_empty() {
        ...
        // compared to re-globbing from the filesystem. NB: each input file is glob-escaped so a
        // path containing glob metacharacters is excluded by exact match, mirroring the original
        // Python `putative - set(input_files)` set subtraction rather than glob interpretation.
        ...
        for f in request.input_files.iter() {
            globs.push(format!("!{}", fs::escape_glob(f)));
        }
...
/// Whether `content` is "blank" for the purpose of `ignore_empty_files`, matching the original
/// Python rule's `fc.content.strip()` emptiness test. Python's `bytes.strip()` strips the six
/// ASCII whitespace bytes `b" \t\n\r\x0b\x0c"` — note this includes vertical tab (`0x0b`), which
/// Rust's `u8::is_ascii_whitespace` deliberately excludes, so match the Python set exactly.
