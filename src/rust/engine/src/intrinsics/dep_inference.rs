...
use pyo3::prelude::{PyModule, PyResult, Python, wrap_pyfunction};
...
use crate::externs::store_dict;
use crate::nodes::{NodeResult, task_get_context};
use crate::python::{Failure, Value};
use crate::{Core, externs};

pub fn register(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(py_bindings::parse_dockerfile_info, m)?)?;
    m.add_function(wrap_pyfunction!(py_bindings::parse_python_deps, m)?)?;
    m.add_function(wrap_pyfunction!(py_bindings::parse_javascript_deps, m)?)?;
...
pub async fn parse_dockerfile_info(deps_request: Value) -> NodeResult<Value> {
...
pub async fn parse_python_deps(deps_request: Value) -> NodeResult<Value> {
    let context = task_get_context();
...
pub async fn parse_javascript_deps(deps_request: Value) -> NodeResult<Value> {
    let context = task_get_context();
...
mod py_bindings {
    ...
    #[pyfunction]
    pub fn parse_dockerfile_info(deps_request: Value) -> PyGeneratorResponseNativeCall {
        native_call(deps_request, super::parse_dockerfile_info)
    }

    #[pyfunction]
    pub fn parse_python_deps(deps_request: Value) -> PyGeneratorResponseNativeCall {
        native_call(deps_request, super::parse_python_deps)
    }

    #[pyfunction]
    pub fn parse_javascript_deps(deps_request: Value) -> PyGeneratorResponseNativeCall {
        native_call(deps_request, super::parse_javascript_deps)
    }
}
