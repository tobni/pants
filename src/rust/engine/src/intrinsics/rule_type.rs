...
use crate::nodes::{
    NodeResult, Snapshot, lift_directory_digest, select, task_get_context, task_get_rule_entry,
    task_get_rule_params,
};
...
/// Graph-dispatches a call from `I` to `O`, the Rust mirror of a Python `@rule`'s `Get`. When run
/// inside a rule body it resolves through the calling rule's own edges (so the caller's params and
/// `@union` scope apply); otherwise it resolves `(O, [I])` as a free-standing query root. The rule
/// graph must contain a path either way: the `#[native_rule]` macro AST-scans bodies for
/// `Implicitly<I, O>` hops and `install_into` registers each as a `Get` on the rule, and every
/// native rule also registers its own `(O, [I])` root.
async fn resolve_implicit<I: RuleType, O: RuleType>(input: I) -> NodeResult<O> {
    ...
    // Resolution strategy, mirroring how a Python `@rule` resolves a `Get`:
    //
    // 1. If we're executing inside a rule body (the common case — set by `rule_scope`), dispatch
    //    the `I → O` hop through *that rule's* own graph edges. This carries the caller's params
    //    into scope, so a hop whose target needs an ambient param — or an `@union` member — is
    //    satisfiable, and two rules that produce `O` from `I` in different contexts stay
    //    disambiguated by which rule is on the stack. Requires the hop to be a registered `Get`
    //    on the rule, which `install_into` derives from the macro's AST scan.
    //
    // 2. Otherwise (no enclosing scope, or a hop reached indirectly through another rule's inlined
    //    public fn that the caller's macro couldn't see), fall back to resolving `(O, [I])` as a
    //    free-standing query root. Native rules each register their own `(product, [inputs])` root,
    //    so self-contained hops always resolve this way.
    let dependency_key = DependencyKey::new(output_type).provided_params([input_type]);
    let scoped = match (task_get_rule_entry(), task_get_rule_params()) {
        (Some(caller_entry), Some(caller_params)) => ctx
            .core
            .rule_graph
            .edges_for_inner(&caller_entry)
            .and_then(|edges| {
                edges.entry_for(&dependency_key).or_else(|| {
                    // The input may be an `@union`: retry with its in-scope members, as
                    // `gen_call` does for by-name calls.
                    let in_scope = input_type.union_in_scope_types()?;
                    edges.entry_for(
                        &DependencyKey::new(output_type)
                            .provided_params([input_type])
            ...
            .map(|entry| {
                ...
                params.extend([input_key.clone()]);
                (params, entry)
            }),
        _ => None,
    };

    let (params, entry) = match scoped {
