...
use pyo3::{Bound, IntoPyObject, PyErr, PyResult, Python, wrap_pyfunction};
...
pub fn register(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(py_bindings::execute_process, m)?)?;

    Ok(())
}

pub async fn execute_process(process: Value, process_config: Value) -> NodeResult<Value> {
...
mod py_bindings {
    ...
    #[pyfunction]
    pub fn execute_process(process: Value, process_config: Value) -> PyGeneratorResponseNativeCall {
        native_call2(process, process_config, super::execute_process)
    }
}
