...
pub fn register(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(py_bindings::docker_resolve_image, m)?)?;

    Ok(())
}

pub async fn docker_resolve_image(docker_request: Value) -> NodeResult<Value> {
...
mod py_bindings {
    ...
    #[pyfunction]
    pub fn docker_resolve_image(docker_request: Value) -> PyGeneratorResponseNativeCall {
        native_call(docker_request, super::docker_resolve_image)
    }
}
