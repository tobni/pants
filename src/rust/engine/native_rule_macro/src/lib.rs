...
/// Optional metadata mirroring the corresponding `@rule` keyword arguments. Parsed from the
/// attribute: `#[native_rule(level = Debug, desc = "...", masked(TypeA, TypeB))]`. Any subset may
/// be given; omitted entries fall back to the same defaults a bare `@rule` uses.
#[derive(Default)]
struct RuleArgs {
...
/// The attribute macro. Accepts the optional `level` / `desc` / `masked` metadata; see [`RuleArgs`].
    ...
    if !attr.is_empty() {
        let parser = syn::meta::parser(|meta| {
            if meta.path.is_ident("level") {
                args.level = Some(meta.value()?.parse()?);
                Ok(())
                ...
                meta.parse_nested_meta(|m| {
                    args.masked.push(m.path.clone());
                    Ok(())
                })
            } else {
                Err(meta.error(
                    "#[native_rule] supports only `level = <Level>`, `desc = \"...\"`, and \
                     `masked(Type, ...)`",
                ))
            }
        });
...
fn expand(input: ItemFn, args: RuleArgs) -> syn::Result<proc_macro2::TokenStream> {
    ...
    // Optional `@rule`-equivalent metadata, defaulting to the same values a bare `@rule` uses.
    let level_tokens = match &args.level {
        Some(level) => quote! { ::log::Level::#level },
        None => quote! { ::log::Level::Trace },
    };
    let desc_tokens = match &args.desc {
        Some(desc) => quote! { ::std::option::Option::Some(#desc.to_string()) },
        None => quote! { ::std::option::Option::None },
    };
    let masked_paths = &args.masked;
    let masked_tokens = quote! {
        vec![#(
            <#masked_paths as crate::intrinsics::rule_type::RuleType>::python_type_id(py),
        )*]
    };
