                ...
                .map(|(stat_symbolic_path, stat)| {
                    ...
                    async move {
                      ...
                      "Maximum link depth exceeded at {l:?} for {stat_symbolic_path:?}"
                                    ...
                                    let dest =
                                        context.canonicalize_link(stat_symbolic_path, l).await?;

                                    Ok(dest.map(|ps| (ps, link_depth + 1)))
                                }
                                Stat::Dir(d) => {
                                    Ok(Some((PathStat::dir(stat_symbolic_path, d), link_depth)))
                                }
                                Stat::File(f) => {
                                    Ok(Some((PathStat::file(stat_symbolic_path, f), link_depth)))
                                }
                            }
                        }
                    }
