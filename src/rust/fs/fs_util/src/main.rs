                ...
                let mtls = match (
                    ...
                    (Some(cert_chain_path), Some(key_path)) => {
                        let key = std::fs::read(key_path).map_err(|err| {
                            format!(
                                "Failed to read mtls-client-key-path from {key_path:?}: {err:?}"
                            )
                        })?;
            ...
            match expect_subcommand(sub_match) {
                ...
                ("save", args) => {
                    ...
                    let file = fs
                        ...
                        .unwrap()
                        .ok_or_else(|| {
                            format!("Tried to save file {path:?} but it did not exist")
                        })?;
                    match file {
                        fs::Stat::File(f) => {
                            let digest =
                        ...
                        o => Err(format!(
                            "Tried to save file {path:?} but it was not a file, was a {o:?}"
                        )
                        .into()),
                    }
                }
